...
while(transforms_recieved == false)
    ...
    try
        origin_to_port_transform = getTransform(tftree, "talos/origin", "talos/acoustics_port_link");
        port_to_startboard_transform = getTransform(tftree, "talos/acoustics_port_link", "talos/acoustics_starboard_link");
...
function [estimated_pinger_position] = run_analysis(port_to_startboard_transform, origin_to_port_transform, measurements, pinger_depth, pinger_pose, speed_of_sound)
    ...
    if(max(size((measurements))) > points_to_use)
        ...
        while(counter > 0 && counter2 > 0)
            ...
            auv_position_previous = [measurements(points_index_array(counter + 1)).auv_origin.transform.translation.x;measurements(points_index_array(counter + 1)).auv_origin.transform.translation.y;measurements(points_index_array(counter + 1)).auv_origin.transform.translation.z];
